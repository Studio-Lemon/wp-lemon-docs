...
| $args | `array` | additional arguments to help you filter the picture element.<br><br><ul><li>**$post**<br>`object` the post object.</li><li>**$loop**<br>`object` the loop object.</li><li>**$card_type**<br>`string` the card type.</li><li>**$attachment_id**<br>`int` the attachment ID.</li><li>**$picture_classes**<br>`string` the picture classes.</li><li>**$image_size**<br>`string` the image size.</li><li>**$image_sizes**<br>`string` the image sizes attribute.</li><li>**$focalpoint**<br>`bool` whether or not to use the focal point.</li></ul> |
...
## wp-lemon/filter/block/card-grid/default-card

Filters the default card block for the card grid template.

<div class="table-responsive">

| Name | Type | Description |
| --- | --- | --- |
| $default_card | `string` | The default card block name. |
...
```php
add_filter('wp-lemon/filter/block/card-grid/default-card', function ($default_card) {
    return 'acf/icon-card';
});
```
...
**since** 5.63.0 Use this filter to control all admin controller features from one place.
...
- `widgets.support` (bool): Show/hide the support dashboard widget.
- `widgets.team` (bool): Show/hide the remote team dashboard widget.
  for support content. When empty, fallback logic is used.
- `support.capability` (string): Required capability to submit support
  requests via the dashboard form.
- `support.recipient` (string): Recipient email address for support mail.
- `support.email_subject_prefix` (string): Prefix prepended to support
  email subjects.
- `team.title` (string): Team widget heading shown in dashboard.
- `team.description` (string): Optional short intro text.
- `team.endpoint_url` (string): Remote JSON endpoint with team data.
...
## wp-lemon/filter/navwalker/link-attributes
...
Filters the context before rendering the cards in a specific AJAX query.
...
```php
add_filter('wp-lemon/filter/ajax-query/my-custom-action/context', function ($context, $instance) {
  /** @var Generic_Ajax_Query $instance */
  $context['custom_data'] = get_option('my_custom_option');
  return $context;
}, 10, 2);
```

## "wp-lemon/filter/ajax-query/{$this->card\_type}/rendered-html/before

Filters the rendered HTML before it gets compiled.

This allows developers to add html before the rest of the loop is rendered.

<div class="table-responsive">

| Name | Type | Description |
| --- | --- | --- |
| $rendered_html | `string` | The HTML rendered so far. |
| $context | `array` | The context array used to render the next card. |
| $instance | `\Generic_Ajax_Query` | The current instance of the class. |
...
```php
add_filter('wp-lemon/filter/ajax-query/news/rendered-html/before', function ($rendered_html, $context, $instance) {
  /** @var Generic_Ajax_Query $instance */
  return Timber::compile('components/cards/crd-news-intro.twig', $context) . $rendered_html;
}, 10, 3);
```

## "wp-lemon/filter/block/node-overview/{$this->card\_type}/delay-multiplier

Filters the delay multiplier for the delay of the cards.

You can use this to reset the delay multiplier after for example starting the next row of cards when you insert an additional card in front of the loop.
...
| Name | Type | Description |
| --- | --- | --- |
| $delay_multiplier | `int` | The current delay multiplier. |
| $counter | `int` | The current loop counter. |
| $instance | `\Generic_Ajax_Query` | The current instance of the class. |
...
```php
add_filter('wp-lemon/filter/block/node-overview/news/delay-multiplier', function ($delay_multiplier, $counter, $instance) {
  /** @var Generic_Ajax_Query $instance */
  if (3 === $counter) {
    return 1; // Reset the delay multiplier after the 3rd card.
  }
  return $delay_multiplier;
}, 10, 3);
```

## "wp-lemon/filter/ajax-query/{$this->card\_type}/templates

Filters the templates used to render a card before it gets compiled.
...
| Name | Type | Description |
| --- | --- | --- |
| $templates | `array` | The array of template paths, in order of priority. |
| $context | `array` | The context array used to render the template. |
| $counter | `int` | The current loop counter. |
| $instance | `\Generic_Ajax_Query` | The current instance of the class. |
...
```php
add_filter('wp-lemon/filter/ajax-query/news/templates', function ($templates, $context, $counter, $instance) {
  array_unshift($templates, 'components/cards/crd-news-custom.twig');
  return $templates;
}, 10, 4);
```

## "wp-lemon/filter/ajax-query/{$this->card\_type}/context
...
```php
add_filter('wp-lemon/filter/ajax-query/news/context', function ($context, $counter, $instance) {
  $context['custom_data'] = get_option('my_custom_option');
  return $context;
}, 10, 3);
```
...
## wp-lemon/filter/analytics/tagmanager/head-endpoint

Filters the endpoint used for the Google Tag Manager script in the head.
...
| Name | Type | Description |
| --- | --- | --- |
| $tagmanager_head_endpoint | `string` | The endpoint for the Google Tag Manager head script. Default 'https://www.googletagmanager.com/gtm.js?id='. |
...
```php
add_filter('wp-lemon/filter/analytics/tagmanager/head-endpoint', function () {
    return 'https://www.googletagmanager.com/gtm.js?id=';
});
```

## wp-lemon/filter/analytics/tagmanager/body-endpoint

Filters the endpoint used for the Google Tag Manager noscript iframe in the body.
...
| Name | Type | Description |
| --- | --- | --- |
| $tagmanager_body_endpoint | `string` | The endpoint for the Google Tag Manager body noscript iframe. Default 'https://www.googletagmanager.com/ns.html'. |
...
```php
add_filter('wp-lemon/filter/analytics/tagmanager/body-endpoint', function () {
