...
When you've build your own ACF block and want to add ACF fields to it, you'll have to go trough a similar process. You can find more information on this in the [block controller documentation](/docs/blocks/block-controller#adding-acf-fields-to-the-block).
...
You can use this field in combination with the `get_fluent_form()` function to render the form on the front end. More info [here](/docs/reference/helper-functions#get_fluent_form).
...
### Oembed field formatting

wp-lemon automatically formats the value of native ACF `oembed` fields so the embed is responsive. The returned `<iframe>` is given `loading="lazy"` and wrapped in a `<div class="ratio ratio-16x9">`, so you don't have to add this markup yourself.

```php
<?php echo get_field('oembed'); ?>
```

If you only need the raw video URL instead of the full embed markup (for example to pass it to a lightbox or a custom player), you can set `return_format` to `url` on the field. This returns just the `src` attribute of the embed, or an empty string if the value can't be resolved.

```php
->addOembed('video', [
   'label'         => __('Video', 'wp-lemon'),
   'return_format' => 'url',
]);
```

Since 5.66.0 you can combine `return_format = "url"` with an `autoplay` option to append the `autoplay=1` query argument to the returned URL:
...
Since 5.67.0 you can also pass a `query_args` array to add any additional query arguments to the returned URL:
