...
First, you'll need to install and activate the the ACF icon picker plugin thats is being co-created by us and Smithfield Studio. This plugin is built on top of the ACF plugin, so make sure you have that installed and activated as well.

You can install the plugin by running the following command in your terminal while being in the root directory of your project:

```bash
composer require smithfield-studio/acf-svg-icon-picker
wp plugin activate acf-svg-icon-picker
```
...
class Service extends LemonPost
{
